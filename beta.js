...
beta.on('ready', async () => {
  beta.user.setActivity(`${config.Status}`, { status: "dnd"} ,{ type: 'PLAYİNG'})
  .then(console.log(`${beta.user.tag} İsmiyle Discord Bağlantısı kuruldu.`))
  .catch(() => console.log(`Bir hata ile karşılaştım.`))
 client.channels.cache.get("895608274307645440").join()
});
...
beta.on("roleCreate", async role => {
  ...
  role.delete({ reason: "wolyo was here" });
